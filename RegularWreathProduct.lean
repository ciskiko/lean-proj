...
This file defines the regular wreath product of groups, and the canonical maps in and out of the product.
The regular wreath product of `D` and `Q` is the product of sets with the group
...
lemma mu_eq {p n :ℕ} [Fact (Nat.Prime p)]: (p ^ n).factorial.factorization p = ∑ i ∈ Finset.range n, p ^ i := by
  ...
  | succ n h =>
    ...
    have h_compl : (∏ k ∈ (Finset.range (p^(n+1)) \ S), (k+1)).factorization p = 0 := by
        ...
        have hhh : ¬ (x ∈ { k ∈ Finset.range (p^(n+1)) | p ∣ (k+1)}) ↔ ¬ (x ∈ Finset.range (p^(n+1)) ∧ (p ∣ (x+1))) :=
...
def f {p n : ℕ} [Fact (Nat.Prime p)] (D : Sylow p (Equiv.Perm (Fin (p^n))))
  (G : Type) [Finite G] [Group G] [IsCyclic G] (h: Nat.card G = p):
    D ≀ᵣ G →* Equiv.Perm (Fin (p^(n+1))) :=
     (aux ((Equiv.prodCongrRight fun _ => (Finite.equivFinOfCardEq h)).trans finProdFinEquiv)).comp (RegularWreathProduct.toPerm D G (Fin (p^n)))
  ...
  P ≃* (IteratedWreathProduct Z_p n) := by
    ...
    | succ n h_n =>
        ...
        have g: (IteratedWreathProduct Z_p (n+1)) ≃* MonoidHom.range (f P' Z_p h) :=
          (RegularWreathProduct.congr (h_n P').symm (MulEquiv.refl Z_p)).trans (MonoidHom.ofInjective (f_injective P' Z_p h))
        have sylow_card: Nat.card (MonoidHom.range (f P' Z_p h)) = p ^ (Nat.card (Equiv.Perm (Fin (p^(n+1))))).factorization p := by
